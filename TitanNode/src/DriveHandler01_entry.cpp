...
void DriveHandler01_entry(void)
{
    int i;
    move the_move;
    DH1 = new SingleMotorLinearDrive();
//    tx_thread_sleep (5000);
    while(ptr_ProcessManager==NULL){
        tx_thread_sleep(100);
    }
    ptr_ProcessManager->ptr_drive_handlers[DRIVE_HANDLER_INDEX_01] = DH1;
...
/*
    for(i=0;i<0;i++){
        ...
        if(i<6){
            the_move.move_type = MOVE_TYPE_CLOCK_COUNT_REV;
            the_move.frequency = 1000*(5 - i);
            the_move.clock_cycle_target = 4800;
        }else{
            the_move.move_type = MOVE_TYPE_CLOCK_COUNT_FWD;
