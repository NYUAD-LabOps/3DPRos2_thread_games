...
static uint8_t DriveHandler01_stack[4096] BSP_PLACE_IN_SECTION_V2(".stack.DriveHandler01") BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
void DriveHandler01_create(void)
{
    ...
    err = tx_thread_create (&DriveHandler01, (CHAR*) "DriveHandler01", DriveHandler01_func, (ULONG) NULL,
                            &DriveHandler01_stack, 4096, 3, 3, 1, TX_DONT_START);
    if (TX_SUCCESS != err)
    {
